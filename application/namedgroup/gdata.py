"""
名前の付けられた群の共役類の特性を記録する。
"""
from ..calc.conjugacy import ConjugacyCount
...
class NamedGroupData(object):
    """
    登録されている非可換群を扱うクラス。
    
    """   
    @classmethod
    def groups(cls, order: int) -> 'tuple[NamedGroup]':
        """
        登録されている群から、指定の位数の群の一覧を取得する。
        ...
        tuple[NamedGroup]
            指定の位数の群の一覧。
            該当する群が登録されていない場合には、空のタプル。

        """
        return (cls._named_group_dict[order] 
                if order in cls._named_group_dict
                else tuple())
    ...
    #### 384 -> (候補): Delta384
    Delta384 = ((1, 1, 1), (2, 3, 1), (2, 24, 1), (3, 128, 1), (4, 3, 2),
                (4, 6, 1), (4, 24, 1), (8, 3, 4), (8, 6, 6), (8, 24, 2), 
                (16, 24, 4))
    _named_group_dict[384] = (
        NamedGroup("Delta(384)",Delta384),
        )
